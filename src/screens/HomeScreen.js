...
function HomeScreen({ navigation, addNotification }) {
  const [workplaceText, setWorkplaceText] = useState('');

  const addNotificationAndWorkplace = () => {
    if (workplaceText.trim() !== '') {
      addNotification('알림 ', workplaceText);
      setWorkplaceText('');
    }
  };

  const gotowork = () => {
    addNotification('출근', '박지완님이 출근했습니다.');
  };
  
  const gotohome = () => {
    addNotification('퇴근', '박지완님이 퇴근했습니다.');
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView}>
        <TouchableOpacity 
          ...
          onPress={() => alert('메모')}
        >
          <Text style={styles.cardDate}>05.08(금)</Text>
        ...
        >
          ...
          <Text style={styles.cardSubtitle}>예정 근무 : 2024-05-08</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.footerButton} onPress={() => alert('근무지 추가')}>
          <Text style={styles.footerText}>근무추가</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.footerButton} onPress={() => alert('근무지 추가')}>
          <Text style={styles.footerText}>근무지</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.footerButton} onPress={() => alert('근무지 추가')}>
